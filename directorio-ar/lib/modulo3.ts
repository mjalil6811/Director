...
export const PREGUNTAS_M3 = [
  ...
  {
    texto: "¿Cuál es el nivel de complejidad del gobierno actual?",
    opciones: ["Baja (todo informal)", "Media (algunas reglas)", "Alta (procesos definidos)", "Muy alta (gobierno profesional)"],
  },
];
...
export function calcularModulo3(respuestas: number[]): Modulo3Resultado {
  ...
  const totalScore = ranked.reduce((sum, [, v]) => sum + v, 0);
